...
new class extends Component {
    ...
    {
        return ['todos' => Auth::user()->todos()->get()];
    }
}; ?>
    ...
    @foreach ($todos as $todo)
        <div wire:key='{{ $todo->id }}' class="flex items-center space-x-4 space-y-2 ">
            <div>
                ...
                <button wire:click='deleteTodo({{ $todo->id }})' class="px-4 py-2 font-bold text-white bg-red-500 rounded hover:bg-red-700 " >DEL<button>
        </div>
    @endforeach
</div>
